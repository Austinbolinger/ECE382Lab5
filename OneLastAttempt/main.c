	...
	while (1) {
		...
		if (newIrPacket) {
			...
			if (irpacket3 == PWR) {
				...
				//P1OUT ^= BIT6;
				//on
			} else if (irpacket3 == ZERO) {
				codeMatches = 0;
				if (y <= 6) {
					y = y + 1;
				}
			} else if (irpacket3 == ONE) {
				codeMatches = 1;
				if (x >= 1) {
					x = x - 1;
				}
			} else if (irpacket3 == TWO) {
				codeMatches = 2;
				//P1OUT ^= BIT0;
				if (y >= 1) {
					y = y - 1;
				}
			} else if (irpacket3 == THR) {
				codeMatches = 3;
				if (x <= 10) {
					x = x + 1;
				}
			} else if (irpacket3 == VOL_UP) {
				codeMatches = 5;
				//P1OUT |= BIT0;
			} else if (irpacket3 == VOL_DW) {
				codeMatches = 6;
				//P1OUT &= ~BIT0;
			} else if (irpacket3 == CH_UP) {
				codeMatches = 7;
//				//P1OUT |= BIT6;
//				if (color) {
//					color = FALSE;
...
__interrupt void pinChange(void) {
	...
	case 0:						// !!!!!!!!!NEGATIVE EDGE!!!!!!!!!!
		...
		if (pulseDuration > minStartPulse && pulseDuration < maxStartPulse) {
			...
			//irpacket = 1;
			//irpacket2 = (irpacket2 << 1) | 1;
			packetData2[packetIndex] = 1;
			packetIndex++;

		} else if (pulseDuration > minLogic0Pulse
				&& pulseDuration < maxLogic1Pulse && packetIndex < 33) {
			//irpacket = 0;
			//irpacket2 = (irpacket2 << 0);
			packetData2[packetIndex] = 0;
			packetIndex++;
		}

		//packetData2[packetIndex] = pulseDuration;
		//packetIndex++;
		//packetData[packetIndex] = irpacket2;
		//packetIndex++;
